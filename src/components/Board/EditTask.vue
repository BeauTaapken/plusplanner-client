    ...
    <v-card class="createTask">
      <v-form>
        <v-container>
          ...
          <v-text-field id="taskdescription" label="Description" filled shaped :value="description"></v-text-field>
          <v-menu>
            <template v-slot:activator="{ on }">
              <v-text-field v-model="date" label="End Date" readonly v-on="on" :value="endDate"></v-text-field>
            </template>
            <v-date-picker
              v-model="date"
              @input="menu2 = false"
...
export default {
  ...
  methods: {
    ...
    save: function() {
      ...
      this.$parent.$parent.$parent.$parent.$parent.itemArray.push(
        JSON.parse(newItem)
      );
      this.overlay = !this.overlay;
    }
